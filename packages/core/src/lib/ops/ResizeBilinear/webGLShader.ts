...
import { WebGLDataTextureArray, WebGLOpNode } from "../../types";

type ResizeBilinearParams = {}
...
export const resizeBilinearWebGLShader = (opNode: WebGLOpNode): string => {
  ...
  const {
    ...
    numFilters,
    activation
  } = opNode.opParams as any; // ResizeBilinearParams;
  ...
  if (!output) {
    throw new Error("ResizeBilinear operation must have an output");
  }
  ...
  console.log("input original shape", input.originalShape);
  console.log("output original shape", output.originalShape);

  // baaaaaaaaaaaaaaahhhhh here you were using both shape and texture dims??? whyyy
  // XXX: You replaced inputDims.z with paddedInputDepth.
...
void main() {
  ...
  int firstFilterNum = target0Output0RealXYZ.z;

  for(int x = 0; x < paddedKernelDims.x; x++) {
    // The filters are packed along the x axis.
    // So we need to multiply the x index by the number of filters to get the correct filter index.
    int packedX = firstFilterNum + x * numFilters;
    ...
    for(int y = 0; y < paddedKernelDims.y; y++) {
      ...
      for (int z = 0; z < paddedKernelDims.z / 4; z++) {
        ...
        vec4 ${weightsVarName}0 = texelFetch(${weights[0].uniformName}, ivec3(packedX, y, z), 0);
        vec4 ${weightsVarName}1 = texelFetch(${weights[0].uniformName}, ivec3(packedX + 1, y, z), 0);
        vec4 ${weightsVarName}2 = texelFetch(${weights[0].uniformName}, ivec3(packedX + 2, y, z), 0);
        vec4 ${weightsVarName}3 = texelFetch(${weights[0].uniformName}, ivec3(packedX + 3, y, z), 0);
        ...
        ${loop(outputNumberOfTextures, (i: number) => `
        ivec3 input${i}TextureXYZ = convertFlatToTextureArray3D(input${i}FlatIndex + z*4, inputTextureArrayDims.x, inputTextureArrayDims.y, inputTextureSpatialSize);
        vec4 input${i}Values = is${i}InBounds * texelFetch(${input.uniformName}, input${i}TextureXYZ, 0);

        ${resultVarName}${i}.x += dot(input${i}Values, ${weightsVarName}0);
        ${resultVarName}${i}.y += dot(input${i}Values, ${weightsVarName}1);
        ${resultVarName}${i}.z += dot(input${i}Values, ${weightsVarName}2);
        ${resultVarName}${i}.w += dot(input${i}Values, ${weightsVarName}3);

        // For debugging:
        // result0 = ${weightsVarName}2;
        // break;
        `)}
      }
    // break;
